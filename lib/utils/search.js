...
const getWordsToHighlight = searchTerm => {
  ...
  const allWords = words.concat(foundSynonyms).filter(x => x);
  const upcasedWords = allWords.map(word => word.toUpperCase());
  const capitalisedWords = allWords.map(word => word.charAt(0).toUpperCase() + word.slice(1));

  return allWords.concat(upcasedWords).concat(capitalisedWords);
};
