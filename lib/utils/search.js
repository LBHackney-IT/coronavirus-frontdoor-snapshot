...
const filterByCategories = (ids, items) => {
  if (!ids || !ids.length) return items;

  return items.filter(category => ids.includes(category.id));
};
...
const weightByCategories = (ids, items) => {
  return JSON.parse(JSON.stringify(items)).map(item => {
    item.resources = item.resources.map(resource => {
      resource.weight = getCategoryWeight(ids, items, resource);
      return resource;
    });
    return item;
  });
...
const getCategoryWeight = (ids, categorisedResources, resource) => {
  ...
  const selectedCategoriesResourceIsIn = ids.filter(id => {
    return categoriesResourceIsIn.includes(id);
  });

  const unselectedCategoriesResourceIsIn = categoriesResourceIsIn.filter(id => {
    return !ids.includes(id);
  });

  let percentageMatch = (selectedCategoriesResourceIsIn.length / ids.length) * 100;
