import { getSearchResults, getWordsToHighlight, filterByCategories } from 'lib/utils/search';
...
describe('search', () => {
  ...
  describe('ordering by weight within category', () => {
    it('weights services lower if they provide for unselected categories', () => {
      ...
      const onlyExistsInSelectedCategory = weightedResults[0].filter(x => x.id === 1);
      const existsInSelectedAndUnselectedCategory = weightedResults[1].filter(x => x.id === 4);

      expect(onlyExistsInSelectedCategory.weight).toBe(15);
      expect(existsInSelectedAndUnselectedCategory.weight).toBe(10);
    });
    ...
    it('weights services higher if they exist in multiple selected categories', () => {
      ...
      const existsInOneSelectedCategory = weightedResults[0].filter(x => x.id === 1);
      const existsInBothSelectedCategories = weightedResults[1].filter(x => x.id === 4);

      expect(existsInBothSelectedCategories.weight).toBe(25);
      expect(existsInOneSelectedCategory.weight).toBe(15);
    });
  });
