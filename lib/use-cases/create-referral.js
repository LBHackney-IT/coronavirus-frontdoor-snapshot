...
export default class CreateReferral {
  constructor({ referralGateway, sendReferralEmail, sendReferralReceiptEmail }) {
  ...
  async execute({
    ...
    const referral = await this.referralGateway.create({
      ...
      systemIds,
      emails: {
        referral_email_id: referral_email.response.data?.id,
        referral_receipt_email_id: referral_receipt_email.response.data?.id
      }
    });
