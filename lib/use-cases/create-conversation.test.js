...
describe('Create Conversation use case', () => {
  ...
  const sendSignpostLetter = {
    execute: jest.fn(() => ({ response: { status: 201, data: { id: 'fake_id' } } }))
  };
  const sendSignpostReceiptEmail = {
    execute: jest.fn(() => ({ response: { status: 201, data: { id: 'fake_receipt_id' } } }))
  };
  ...
  it('creates a new conversation', async () => {
    ...
    expect(conversationsGateway.create).toHaveBeenCalledWith({
      ...
      emails: {
        ...
        conversationReceiptEmailId: 'fake_receipt_id'
      },
      letters: {
        conversationLetterId: undefined
      }
    });
  ...
  it('sends summary by letter conversation', async () => {
    const result = await createConversation.execute(
      {
        resident: {
          firstName,
    ...
    expect(sendSignpostEmail.execute).not.toHaveBeenCalled();
    expect(sendSignpostLetter.execute).toHaveBeenCalledWith({
