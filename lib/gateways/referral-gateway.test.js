...
describe('ReferralGateway', () => {
  ...
  describe('getByIdForAuthorisedEmail', () => {
    it('throws an error if id is null', async () => {
      const referralGateway = new ReferralGateway({ client, tableName });

      await expect(async () => {
        await referralGateway.getByIdForAuthorisedEmail({ id: null });
      }).rejects.toThrow(ArgumentError);
    ...
    it('can get a referral', async () => {
      ...
      client.query = jest.fn(() => ({
        promise: jest.fn(() => ({
          Items: [{ id, resident: { firstName, lastName }, referredBy: email }]
        }))
      }));
      const expectedRequest = {
        TableName: tableName,
      ...
      const result = await referralGateway.getByIdForAuthorisedEmail({ id, email });
    ...
    it('can return error if referral does not exist', async () => {
      client.query = jest.fn(() => ({
        promise: jest.fn(() => ({ Items: [] }))
      }));
      const referralGateway = new ReferralGateway({ client, tableName });

      const result = await referralGateway.getByIdForAuthorisedEmail({ id: '1', email: 'fake' });

      expect(result.error).toBe(`No referral found with id "1"`);
    });
  });
  ...
  describe('get', () => {
    ...
    it('can get a referral', async () => {
      ...
      client.query = jest.fn(() => ({
        promise: jest.fn(() => ({
          Items: [{ id, resident: { firstName, lastName } }]
        }))
      }));
      const expectedRequest = {
        TableName: tableName,
    ...
    it('can return null if referral does not exist', async () => {
      ...
      const result = await referralGateway.get({ id: '1', email: 'fake' });
