...
const ReferralsTable = ({ referrals }) => {
  return (
    <>
      <p>Displaying {referrals.length} record(s)</p>
          ...
          {referrals.map((referral, index) => {
            ...
            return (
              <tr
                ...
                key={`referrals-table_row-${index}`}
                data-testid="referrals-table-row">
                <td className="govuk-table__cell" data-testid="referrals-table-recerence-number">
                  <a href={`${process.env.NEXT_PUBLIC_URL}/referrals/${referral.id}`}>
                    {referral.referenceNumber || `no ref`}
                  </a>
                </td>
